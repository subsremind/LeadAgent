...
import { getOrganizationsForSession, getPurchasesForSession, getSession, } from "@shared/lib/middleware-helpers";
...
export default async function middleware(req) {
    ...
    if (pathname.startsWith("/app")) {
        ...
        if (!session) {
            return NextResponse.redirect(new URL(withQuery("/auth/login", {
                redirectTo: pathname,
            }), origin));
        }
        if (appConfig.users.enableOnboarding &&
            ...
            return NextResponse.redirect(new URL(withQuery("/app/onboarding", {
                redirectTo: pathname,
            }), origin));
        }
        if (!locale ||
        ...
        if (appConfig.organizations.enable &&
            ...
            return NextResponse.redirect(new URL(organization
                ? `/app/${organization.slug}`
                : "/app/new-organization", origin));
        }
        const hasFreePlan = Object.values(appConfig.payments.plans).some((plan) => "isFree" in plan);
        if (((appConfig.organizations.enable &&
