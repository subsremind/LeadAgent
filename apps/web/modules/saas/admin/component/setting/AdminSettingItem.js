...
export function AdminSettingItem({ settingKey, value }) {
    ...
    return (<div className="grid grid-cols-1 gap-4">
      <form onSubmit={onSubmit} className="space-y-4">
        <div>
          ...
          <Input type="text" className="w-full" {...form.register("value")}/>
          {form.formState.errors.value && (<p className="text-red-500 text-sm mt-1">
              {form.formState.errors.value.message}
            </p>)}
        </div>
        
        <div className="flex justify-end">
          <Button variant="secondary" type="submit" loading={submitting || updateSettingMutation.isPending} disabled={!form.formState.isValid}>
