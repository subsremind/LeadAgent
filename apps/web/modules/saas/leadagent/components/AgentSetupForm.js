...
export function AgentSetupForm({ agentSetting, organizationId, onSuccess, }) {
    ...
    return (<Form {...form}>
			<form onSubmit={onSubmit}>
				...
				<div className="col-span-2 w-full flex justify-end p-3">
					<Button variant="primary" type="submit" loading={isGenerating} className="flex items-center space-x-2" onClick={() => {
            ...
            form.setValue("subreddit", "");
				...
				<FormField control={form.control} name="subreddit" render={({ field }) => (<FormItem>
				...
				<FormField control={form.control} name="query" render={({ field }) => (<FormItem>
				...
				{Object.entries(form.formState.errors).length > 0 && (<div className="bg-red-100 p-4 rounded-md mb-4">
						<p className="text-red-800">Please fix the errors below:</p>
						{Object.entries(form.formState.errors).map(([name, error]) => (<div key={name} className="mt-2">
				...
				<div className="col-span-2 w-full flex justify-end p-3">
					<Button variant="primary" type="submit" loading={form.formState.isSubmitting} className="flex items-center space-x-2">
