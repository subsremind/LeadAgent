...
export function OnboardingStep1({ onCompleted }) {
    ...
    const onSubmit = async ({ name, timezone }) => {
				...
				<form className="flex flex-col items-stretch gap-8" onSubmit={form.handleSubmit(onSubmit)}>
					...
					<FormField control={form.control} name="timezone" render={({ field }) => (<FormItem>
							...
							<Popover>
								...
								<FormControl>
									<Button variant="outline" role="combobox" className={cn("w-full pl-3 text-left font-normal bg-transparent !text-foreground border border-input cursor-pointer", // Added cursor-pointer
            !field.value && "text-muted-foreground")}>
                ...
                : "Select timeZone"}
								...
								<Command>
									...
									{Object.entries(groupTimezonesByContinent()).map(([continent, timezones]) => (<CommandGroup heading={continent} key={continent}>
										{timezones.map((timezone) => (<CommandItem value={timezone} key={timezone} onSelect={() => {
                        form.setValue("timezone", timezone);
                    }}>
											{timezone}
											<Check className={cn("ml-auto", timezone === field.value
                        ? "opacity-100"
                        : "opacity-0")}/>
										</CommandItem>))}
									</CommandGroup>))}
