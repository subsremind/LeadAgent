...
export function ActivePlan({ organizationId, }) {
    ...
    const activePlanData = planData[activePlan.id];
    if (!activePlanData) {
        return null;
    }
    const price = "price" in activePlan ? activePlan.price : null;
    return (<SettingsItem title={t("settings.billing.activePlan.title")}>
			<div className="rounded-lg border p-4">
				<div className="">
					...
					{!!activePlanData.features?.length && (<ul className="mt-2 grid list-none gap-2 text-sm">
							{activePlanData.features.map((feature, key) => (<li key={key} className="flex items-center justify-start">
									<CheckIcon className="mr-2 size-4 text-primary"/>
									<span>{feature}</span>
								</li>))}
						</ul>)}

					{price && (<strong className="mt-2 block font-medium text-2xl lg:text-3xl" data-test="price-table-plan-price">
				...
				{"purchaseId" in activePlan && (<div className="mt-4 flex justify-end">
