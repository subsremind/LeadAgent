...
import { activeOrganizationQueryKey, useActiveOrganizationQuery, } from "@saas/organizations/lib/api";
...
export function ActiveOrganizationProvider({ children, }) {
    ...
    const activeOrganizationSlug = params.organizationSlug;
    ...
    const setActiveOrganization = async (organizationSlug) => {
        nProgress.start();
        const { data: newActiveOrganization } = await authClient.organization.setActive(organizationSlug
            ? {
                organizationSlug,
            }
            : {
        ...
        await queryClient.setQueryData(activeOrganizationQueryKey(newActiveOrganization.slug), newActiveOrganization);
        ...
        await queryClient.setQueryData(sessionQueryKey, (data) => {
            return {
                ...data,
                session: {
                    ...data?.session,
    ...
    const activeOrganizationUserRole = activeOrganization?.members.find((member) => member.userId === session?.userId)?.role;
    return (<ActiveOrganizationContext.Provider value={{
            loaded,
            activeOrganization: activeOrganization ?? null,
            activeOrganizationUserRole: activeOrganizationUserRole ?? null,
            isOrganizationAdmin: !!activeOrganization &&
                !!user &&
