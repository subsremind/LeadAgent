...
import { AlertTriangleIcon, ArrowRightIcon, EyeIcon, EyeOffIcon, KeyIcon, MailboxIcon, } from "lucide-react";
...
export function LoginForm() {
    ...
    const onSubmit = async (values) => {
        try {
            ...
            form.setError("root", {
                message: getAuthErrorMessage(e && typeof e === "object" && "code" in e
                    ? e.code
                    : undefined),
            });
        }
    ...
    return (<div>
            ...
            signinMode === "magic-link" ? (<Alert variant="success">
                ...
                config.auth.enablePasswordLogin && (<LoginModeSwitch activeMode={signinMode} onChange={(mode) => form.setValue("mode", mode)}/>)}
                ...
                signinMode === "password" && (<FormField control={form.control} name="password" render={({ field }) => (<FormItem>
												<div className="flex justify-between gap-4">
													<FormLabel>
														{t("auth.signup.password")}
													</FormLabel>

													<Link href="/auth/forgot-password" className="text-foreground/60 text-xs">
                        ...
                        ? "text"
                        : "password"} className="pr-10" {...field} autoComplete="current-password"/>
														<button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute inset-y-0 right-0 flex items-center pr-4 text-primary text-xl">
							...
							<Button className="w-full" type="submit" variant="primary" loading={form.formState.isSubmitting}>
                ...
                (config.auth.enableSignup &&
                    config.auth.enableSocialLogin)) && (<>
							...
							<div className="grid grid-cols-1 items-stretch gap-2 sm:grid-cols-2 ">
								{config.auth.enableSignup &&
                    config.auth.enableSocialLogin &&
                    Object.keys(oAuthProviders).map((providerId) => (<SocialSigninButton key={providerId} provider={providerId}/>))}

								{config.auth.enablePasskeys && (<Button className="w-full sm:col-span-2" onClick={() => signInWithPasskey()}>
