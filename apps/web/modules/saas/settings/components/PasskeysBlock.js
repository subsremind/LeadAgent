...
export function PasskeysBlock() {
        ...
        await authClient.passkey.addPasskey({
            fetchOptions: {
                onSuccess: () => {
                    ...
                    toast.success(t("settings.account.security.passkeys.notifications.addPasskey.success.title"));
                },
                onError: () => {
                    toast.error(t("settings.account.security.passkeys.notifications.addPasskey.error.title"));
                },
            },
        });
    };
    ...
    return (<SettingsItem title={t("settings.account.security.passkeys.title")} description={t("settings.account.security.passkeys.description")}>
			<div className="grid grid-cols-1 gap-2">
				{isPending ? (<>
						<div className="flex gap-2">
					...
					</>) : (passkeys?.map((passkey) => (<div key={passkey.id} className="flex gap-2">
							...
							<Button variant="outline" size="icon" className="shrink-0" onClick={() => deletePasskey(passkey.id)}>
