...
export function getContentStructure({ documents, meta, locale, }) {
    const contentStructure = [];
    function addToContentItemArray(contentItemsArray, subPath, item) {
    ...
    function sortContentItems(items, basePath = "") {
        items.sort((a, b) => {
            if (a.path === "") {
                return -1;
            }
            ...
            const aIndex = Object.entries(meta
                .filter((meta) => meta.path === basePath)
                .sort((page) => (page.locale === locale ? -1 : 1))
                .at(0)?.data ?? {}).findIndex(([key]) => key === a.path.replace(`${basePath}/`, ""));
            const bIndex = Object.entries(meta
                .filter((meta) => meta.path === basePath)
                .sort((page) => (page.locale === locale ? -1 : 1))
                .at(0)?.data ?? {}).findIndex(([key]) => key === b.path.replace(`${basePath}/`, ""));
            // use position index from meta file or put the item at the end of the list
            return ((aIndex > -1 ? aIndex : items.length) -
...
export function getActivePathFromUrlParam(path) {
    return Array.isArray(path) ? path.join("/") : path || "";
}
export function getLocalizedDocumentWithFallback(documents, path, locale) {
    return documents
        .filter((doc) => doc.path === path)
        .sort((doc) => (doc.locale === locale ? -1 : 1))[0];
}
export function slugifyHeadline(headline) {
