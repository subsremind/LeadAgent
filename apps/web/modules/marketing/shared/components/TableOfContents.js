...
export function TableOfContents({ items, }) {
    ...
    return (<div className="w-full max-w-64 self-start rounded-lg border p-4">
			<h3 className="mb-2 font-semibold text-base">
				{t("common.tableOfContents.title")}
			</h3>
			<nav className="list-none space-y-2">
				{items.map((item) => (<a key={item.slug} href={`#${item.slug}`} className={`block text-sm ${`ml-${Math.max(0, item.lvl - 2) * 2}`}`} onClick={(e) => {
                e.preventDefault();
