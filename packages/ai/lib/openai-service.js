...
export class OpenAIService {
     ...
     * 通用的OpenAI聊天完成请求方法
     */
    async chatCompletion(business, config, logOptions) {
        const startTime = Date.now();
        const requestId = nanoid();
        try {
            // 执行API请求 - 使用类型断言解决类型不匹配问题
            const response = await this.openaiClient.chat.completions.create(config);
            ...
            if (config.prompt) {
                ...
                promptText = config.messages.map((msg) => `${msg.role}: ${msg.content}`).join('\n');
            }
            // 构建响应文本用于日志记录
            const responseText = response.choices.map(choice => choice.message?.content || JSON.stringify(choice.message?.function_call || choice.message?.tool_calls || '')).join('\n');
            // 计算成本
     ...
     * 简化的文本生成方法
     */
    async generateText(business, prompt, options) {
    ...
    // 生成查询文本的向量表示
    async generateEmbedding(business, userId, text) {
        const startTime = Date.now();
        const requestId = nanoid();
        try {
     ...
     * 流式响应方法
     */
    async streamChatCompletion(business, config, onChunk, logOptions) {
        const startTime = Date.now();
        const requestId = nanoid();
        let fullResponse = '';
        let totalTokens = 0;
        ...
        try {
            ...
            const stream = await this.openaiClient.chat.completions.create(streamConfig);
            // 处理流式响应
