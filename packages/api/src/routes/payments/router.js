...
import { createCheckoutLink, createCustomerPortalLink, getCustomerIdFromEntity, } from "@repo/payments";
...
export const paymentsRouter = new Hono()
    .basePath("/payments")
    .get("/purchases", authMiddleware, validator("query", z.object({
    ...
    .post("/create-checkout-link", authMiddleware, validator("query", z.object({
    ...
    const { productId, redirectUrl, type, organizationId } = c.req.valid("query");
    const user = c.get("user");
    ...
    const plan = Object.entries(plans).find(([_planId, plan]) => plan.prices?.find((price) => price.productId === productId));
    const price = plan?.[1].prices?.find((price) => price.productId === productId);
    const trialPeriodDays = price && "trialPeriodDays" in price
    ...
    const seats = organization && price && "seatBased" in price && price.seatBased
    ...
    .post("/create-customer-portal-link", authMiddleware, validator("query", z.object({
    purchaseId: z.string().optional(),
    redirectUrl: z.string().optional(),
})), describeRoute({
    tags: ["Payments"],
    summary: "Create a customer portal link",
    description: "Creates a customer portal link for the customer or team. If a purchase is provided, the link will be created for the customer of the purchase.",
    responses: {
        200: {
    ...
    if (purchase.organizationId) {
        const userOrganizationMembership = await getOrganizationMembership(user.id, purchase.organizationId);
